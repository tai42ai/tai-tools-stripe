...
[![CI](https://github.com/tai42ai/tai-tools-stripe/actions/workflows/ci.yml/badge.svg)](https://github.com/tai42ai/tai-tools-stripe/actions/workflows/ci.yml)
...
server, with the `stripe` extra (the curl backing dependency):

```bash
uv add 'tai42-tools-stripe[stripe]'
```
...
```bash
git clone https://github.com/tai42ai/tai-tools-stripe   # next to your app checkout
cd /path/to/your/app
uv add --editable '../tai-tools-stripe[stripe]'
```

A tool imported without the `stripe` extra fails loudly at import with a
copy-pasteable install hint — never a silent skip.

## Catalog

| Tool | Extra | Description |
| --- | --- | --- |
| `create_stripe_checkout` | `stripe` | Creates a Stripe Checkout Session and returns its hosted payment URL, usable directly as the `link` builder for an external ask. |
| `confirm_stripe_payment` | `stripe` | Webhook bridge: answers a paid checkout ask from a projected Stripe event. Not a user/agent tool — it holds the bridge secret. |
| `reconcile_stripe_payments` | `stripe` | Recovery layer: re-answers paid sessions the webhook path lost, re-derived from Stripe's own session list. Not a user/agent tool. |
| `create_stripe_payment_link` | `stripe` | Flexible-amount, non-blocking: mints a hosted payment link with no callback and returns its URL and session id. Deployment-fenced to deterministic flow callers. |
