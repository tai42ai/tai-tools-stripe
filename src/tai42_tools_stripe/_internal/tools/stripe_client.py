...
with a bounded transient retry. Needs the ``stripe`` extra; missing it raises a loud install hint
at import.
...
try:
    ...
    raise ImportError(
        "tai42-tools-stripe 'stripe' tools require the 'stripe' optional dependency "
        "(tai42-kit[curl]). "
        "Install it with: pip install 'tai42-tools-stripe[stripe]'"
    ) from exc
